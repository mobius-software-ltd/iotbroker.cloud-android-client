package com.mobius.software.android.iotbroker.main.iot_protocols.mqtt_sn.parser;
...
import com.mobius.software.android.iotbroker.main.iot_protocols.mqtt_sn.parser.avps.*;
...
import com.mobius.software.android.iotbroker.main.iot_protocols.mqtt_sn.parser.packet.impl.*;
import com.mobius.software.android.iotbroker.main.iot_protocols.mqtt_sn.parser.util.ValuesValidator;
...
public class Parser extends AbstractParser
{
	...
	{
		return this.encode(this.decode(buf));
	}
	...
	public Message decode(ByteBuf buf)
	{
		...
		try
		{
			...
			case REGISTER:
				...
				bytesLeft -= 2;
				int registerPacketID = buf.readUnsignedShort();
				if (!ValuesValidator.validatePacketID(registerPacketID))
					throw new MalformedMessageException(type + " invalid packetID " + registerPacketID);
				bytesLeft -= 2;
				if (!ValuesValidator.canRead(buf, bytesLeft))
				...
				message = new Register(registerTopicID, registerPacketID, registerTopicName);
				break;
			...
			case REGACK:
				...
				int regackPacketID = buf.readUnsignedShort();
				if (!ValuesValidator.validatePacketID(regackPacketID))
					throw new MalformedMessageException(type + " invalid packetID " + regackPacketID);
				ReturnCode regackCode = ReturnCode.valueOf(buf.readByte());
				message = new Regack(regackTopicID, regackPacketID, regackCode);
				break;
			...
			case PUBLISH:
				...
				bytesLeft -= 2;
				int publishPacketID = buf.readUnsignedShort();
				bytesLeft -= 2;
				if (publishFlags.getQos() != QoS.AT_MOST_ONCE && publishPacketID == 0)
					throw new MalformedMessageException("invalid PUBLISH QoS-0 packetID:" + publishPacketID);
				...
				message = new SNPublish(publishPacketID, publishTopic, publishContent, publishFlags.isDup(), publishFlags.isRetain());
				break;
			...
			case PUBACK:
				...
				int pubackPacketID = buf.readUnsignedShort();
				if (!ValuesValidator.validatePacketID(pubackPacketID))
					throw new MalformedMessageException(type + " invalid packetID " + pubackPacketID);
				ReturnCode pubackCode = ReturnCode.valueOf(buf.readByte());
				message = new SNPuback(pubackTopicID, pubackPacketID, pubackCode);
				break;

			case PUBREC:
				int pubrecPacketID = buf.readUnsignedShort();
				if (!ValuesValidator.validatePacketID(pubrecPacketID))
					throw new MalformedMessageException(type + " invalid packetID " + pubrecPacketID);
				message = new SNPubrec(pubrecPacketID);
				break;

			case PUBREL:
				int pubrelPacketID = buf.readUnsignedShort();
				if (!ValuesValidator.validatePacketID(pubrelPacketID))
					throw new MalformedMessageException(type + " invalid packetID " + pubrelPacketID);
				message = new SNPubrel(pubrelPacketID);
				break;

			case PUBCOMP:
				int pubcompPacketID = buf.readUnsignedShort();
				if (!ValuesValidator.validatePacketID(pubcompPacketID))
					throw new MalformedMessageException(type + " invalid packetID " + pubcompPacketID);
			...
			case SUBSCRIBE:
				...
				bytesLeft--;
				int subscribePacketID = buf.readUnsignedShort();
				if (subscribePacketID == 0)
					throw new MalformedMessageException(type + " invalid packetID " + subscribePacketID);
				bytesLeft -= 2;
				...
				message = new SNSubscribe(subscribePacketID, subscribeTopic, subscribeFlags.isDup());
				break;
			...
			case SUBACK:
				...
				int subackPacketID = buf.readUnsignedShort();
				if (!ValuesValidator.validatePacketID(subackPacketID))
					throw new MalformedMessageException(type + " invalid packetID " + subackPacketID);
				ReturnCode subackCode = ReturnCode.valueOf(buf.readByte());
				message = new SNSuback(subackTopicID, subackPacketID, subackCode, subackFlags.getQos());
				break;

			case UNSUBSCRIBE:
				Flags unsubscribeFlags = Flags.decode(buf.readByte(), type);
				bytesLeft--;
				int unsubscribePacketID = buf.readUnsignedShort();
				if (!ValuesValidator.validatePacketID(unsubscribePacketID))
					throw new MalformedMessageException(type + " invalid packetID " + unsubscribePacketID);
				bytesLeft -= 2;
				...
				message = new SNUnsubscribe(unsubscribePacketID, unsubscribeTopic);
				break;

			case UNSUBACK:
				int unsubackPacketID = buf.readUnsignedShort();
				if (!ValuesValidator.validatePacketID(unsubackPacketID))
					throw new MalformedMessageException(type + " invalid packetID " + unsubackPacketID);
			...
			case ENCAPSULATED:
				...
				Message encapsulatedMessage = new Parser().decode(buf);
	...
	public ByteBuf encode(Message message)
	{
		...
		case REGISTER:
			...
			buf.writeShort(register.getPacketID());
		...
		case REGACK:
			...
			buf.writeShort(regack.getPacketID());
		...
		case UNSUBACK:
			...
			buf.writeShort(contableMessage.getPacketID());
			break;
		...
		case SUBSCRIBE:
			...
			buf.writeShort(subscribe.getPacketID());
		...
		case UNSUBSCRIBE:
			...
			buf.writeShort(unsubscribe.getPacketID());
		...
		case ENCAPSULATED:
			...
			buf.writeBytes(new Parser().encode(encapsulated.getMessage()));
			break;
