package com.mobius.software.android.iotbroker.main.iot_protocols.mqtt.parser;
...
import com.mobius.software.android.iotbroker.main.iot_protocols.mqtt.parser.avps.LengthDetails;
...
import com.mobius.software.android.iotbroker.main.utility.StringVerifier;

public class MQParser extends AbstractParser
{
	...
	public Message decode(ByteBuf buf) throws MalformedMessageException, UnsupportedEncodingException
	{
		...
		case CONNECT:
			...
			if (willFlag)
			{
				...
				will = new Will(new MQTopic(willTopic, willQos), willMessage, willRetain);
				if (!will.isValid())
					throw new MalformedMessageException("invalid will encoding");
			}
		...
		case PUBLISH:
			...
			header = new Publish(packetID, new MQTopic(new Text(topicName), qos), data, retain, dup);
			break;
		...
		case SUBSCRIBE:
			...
			List<MQTopic> subscriptions = new ArrayList<MQTopic>();
			while (buf.isReadable())
			{
				...
				MQTopic subscription = new MQTopic(new Text(topic), requestedQos);
				subscriptions.add(subscription);
			}
			if (subscriptions.isEmpty())
				throw new MalformedMessageException("Subscribe with 0 topics");

			header = new Subscribe(subID, subscriptions.toArray(new MQTopic[subscriptions.size()]));
			break;
	...
	public ByteBuf encode(Message header) throws UnsupportedEncodingException, MalformedMessageException
	{
		...
		MessageType type = MessageType.valueOf(header.getType());
		...
		case SUBSCRIBE:
			...
			for (MQTopic subscription : sub.getTopics())
			{
