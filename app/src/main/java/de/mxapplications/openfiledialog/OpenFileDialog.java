...
public class OpenFileDialog extends Dialog {
    ...
    public void test() {
        ...
        {
            ArrayList files = (ArrayList) new FileItem("/", true).listChildren();
            Log.i("TAG", String.valueOf(files.size()));
        }


        {
            ArrayList files = (ArrayList) new FileItem("/storage", true).listChildren();
            Log.i("TAG", String.valueOf(files.size()));
        }


        {
            ArrayList files = (ArrayList) new FileItem("/storage/emulated", true).listChildren();
            Log.i("TAG", String.valueOf(files.size()));
        }


        {
            ArrayList files = (ArrayList) new FileItem("/storage/emulated/0", true).listChildren();
            Log.i("TAG", String.valueOf(files.size()));
        }

        {
            ArrayList files = (ArrayList) new FileItem("/storage/emulated/0/Android", true).listChildren();
            Log.i("TAG", String.valueOf(files.size()));
        }
    ...
    private void init(){
        ...
        getContext().getTheme().resolveAttribute(R.color.action_bar_text_color, typedValue, true);
    ...
    private View createView(){
        ...
        layout.setBackgroundColor(getContext().getResources().getColor(R.color.action_bar_text_color));
        ...
        mPathTextView.setMaxLines(1);
        linearLayoutParams = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            mPathTextView.setTextAppearance(android.R.style.TextAppearance_Small);
        }else {
            mPathTextView.setTextAppearance(getContext(), android.R.style.TextAppearance_Small);
        }
    ...
    private class FileListRecyclerViewAdapter extends RecyclerView.Adapter<FileListRecyclerViewAdapter.ViewHolder>{
        ...
        public void onBindViewHolder(final FileListRecyclerViewAdapter.ViewHolder holder, int position) {
            ...
            holder.mView.setBackgroundDrawable(getContext().getResources().getDrawable(R.drawable.pressed_button));
                ...
                public void onClick(View v) {
                    if (holder.mFileItem.isDirectory) {
                        ...
                        if (mOnCloseListener != null) {
                            ...
                            mOnCloseListener.onOk(mSelectedFile.absolutePath);
                        }
                    }
                }
            });
